...
Atlas.JOINT_LENGHTS = cg.get_robot_config_1(                        # Specify the lenghts of each joint as an array of floats in [meter], default is [0.1] for each join
...
Atlas.Tgoals = SE3([                                                # Specify the goal poses of the robot as an array of SE3 objects, default is 
            SE3(0.5, 118.724, 0),                                   # SE3([SE3(0.4, 0.4, 0.4), SE3(0.5, 0.5, 0.5), SE3(0.6, 0.6, 0.6)]) * SE3.Rx(180, 'deg')
            SE3(0.5, 0.5, 0.5),                                     # The  * SE3.Rx(180, 'deg') is used to rotate the goal poses 180 degrees around the x-axis.
            SE3(0.6, 0.6, 0.6)
        ]) * SE3.Rx(180, 'deg')
...
for Tg in Atlas.Tgoals:
    solutions = (Atlas.JOINT_LENGHTS.ikine_LM(Tg, q0=Atlas.JOINT_LENGHTS.qr, mask=[1, 1, 1, 0.5, 0.5, 0.5])) # Calculate the inverse kinematics for each goal pose
    for i in range(len(solutions.q)):
        solutions.q[i] = int((solutions.q[i] % (2 * pi)) * 4095 / (2 * pi))   # Normalize the joint values to be between 0 and 2*pi
    print(solutions)
